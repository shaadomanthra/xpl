...
<button type="button" class="btn btn-lg btn-primary btn-sm mt-4 runcode runcode_{{($i+1)}}" data-namec="{{\auth::user()->username}}_{{$exam->slug}}_{{($i+1)}}" data-testcase="1" data-qslug="{{$question->slug}}" data-qno="{{($i+1)}}" data-url="{{ route('runcode') }}" data-lang="clang" data-name="code_{{($i+1)}}" data-test="{{$exam->slug}}" data-c="1">Compile & Test</button>
...
<div class="alert alert-important alert-warning mt-3">Kindly submit the code for each question to validate the testcases. Mark is awarded only to the question which pass all the test cases.<br>
<b>Note:</b> In java programming, the classname has to be named Main only.</div>
