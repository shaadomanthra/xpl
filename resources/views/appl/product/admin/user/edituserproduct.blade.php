...
@include('flash::message')
  <div class="card">
    <div class="card-body">
      ...
      <form method="post" action="{{route('admin.user.product.update',[$user->id,$product->id])}}" >
      
      <div class="form-group">
        <label for="formGroupExampleInput ">Product</label>
        <select class="form-control course_data" name="product_id">
          @foreach($products as $p)
          <option value="{{$product->id}}" @if(isset($product))@if($p->id == $product->id) selected @endif @endif>{{$product->name}}</option>
      ...
      <button type="submit" class="btn btn-primary">Confirm Updation</button>
