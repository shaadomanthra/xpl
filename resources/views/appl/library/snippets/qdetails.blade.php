<div class="card">
	<div class="card-header">{{ strtoupper($question->reference) }} <span class="btn view badge badge-warning" data-item="details">hide</span>
		...
		<span class="float-right">
			...
			@endcan
			@can('update',$question)
			<a href="{{ route('lquestion.edit',['repo'=>$repo->slug,'question'=>$question->id]) }}">
	...
	<div class="card-body details">
		...
		@if(count($question->structures)!=0)
		<div class="bg-light p-1 rounded border mb-2 mt-3">structures</div>
		<div class="">
			@foreach($question->structures as $structure)
			<a href="{{ route('structure.show',[$repo->slug,$structure->slug])}}">
				{{ $structure->name }}
			</a><br>
			@endforeach
		</div>
		...
		<div class="">
			...
			<div class="col-6">
				<a href="{{ route('ltag.show',[$repo->slug,$tag->id])}}">
				{{ $tag->value }}
				</a>
			</div>
			</div>
		...
		<form method="get" action="{{route('lquestion.show',[$repo->slug,$question->id])}}" >
