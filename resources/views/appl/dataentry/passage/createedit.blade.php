...
@section('content')
  ...
  <div class="card">
    <div class="card-body">
      ...
      @if($stub=='Create')
      <form method="post" action="{{route('passage.store',$project->slug)}}" >
      @else
      <form method="post" action="{{route('passage.update',[$project->slug,$passage->id])}}" >
      @endif  
      ...
      <div class="form-group">
        ...
        <select class="form-control" name="status">
          <option value="0" @if(isset($passage)) @if($passage->status==0) selected @endif @endif >Draft</option>
          <option value="1" @if(isset($passage)) @if($passage->status==1) selected @endif @endif >Published</option>
        </select>
      </div>
