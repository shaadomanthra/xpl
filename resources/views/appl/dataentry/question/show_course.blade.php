  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="exampleModalLabel">Add Question : Project</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <div class="modal-body">
        <h3 >Category Name : {{ request()->session()->get('topic_name') }}</h3>
        <p> Project Name : {{ request()->session()->get('module_name') }}</p>
      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-secondary" data-dismiss="modal">Close</button>
        
        <a href="{{ route('question.copy',$question->id)}}?mode=project">
        <input type="hidden" name="_token" value="{{ csrf_token() }}">
