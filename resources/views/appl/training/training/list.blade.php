      ...
      <div class="d-flex align-items-center">
        ...
        <div class="flex-shrink-0 mr-4 symbol symbol-65 symbol-circle">
          ...
          <div class="d-flex flex-column mr-auto">
            <!--begin: Title-->
            <a href="{{route('training.show',$obj->slug)}}" class="card-title text-hover-primary font-weight-bolder font-size-h5 text-dark mb-1">{{$obj->name}}</a>
        ...
        <!--end::Content-->
        <!--begin::Text-->
        <p class="mb-7 mt-3">{{substr(strip_tags($obj->details),0,200)}}</p>
        <!--end::Text-->
        <!--begin::Blog-->
        <div class="d-flex flex-wrap">
          <!--begin: Item-->
          <div class="mr-12 d-flex flex-column mb-7">
            <span class="font-weight-bolder mb-4">Sessions</span>
            <span class="font-weight-bolder font-size-h5 pt-1">{{$obj->sessions}}</span>
            </div>
            <!--end::Item-->
            
              <!--begin::Item-->
              <div class="d-flex flex-column flex-lg-fill float-left mb-7">
                <span class="font-weight-bolder mb-4">Participants</span>
                <div class="symbol-group symbol-hover">
                  @foreach($obj->users as $k=>$u)
                  <div class="symbol symbol-30 symbol-circle" data-toggle="tooltip" title="{{$u->name}}">
                    <img alt="Pic" src="@if($u->getImage()) {{ ($u->getImage())}}@else {{ Gravatar::src($u->email, 150) }}@endif" />
                  </div>
                    @if($k==4)
                      @if($obj->users->count()>5)
                      <div class="symbol symbol-30 symbol-circle symbol-light">
                        <span class="symbol-label font-weight-bold">{{($obj->users->count()-4)}}+</span>
                      </div>
                      @endif
                      @break
                    @endif
